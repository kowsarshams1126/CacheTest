...
//
        for (int i = 0; i <32 ; i++) {
            And a11=new And("ss",getInput(i),Simulator.trueLogic);
            memory.setInput(
                    i + 2, Simulator.falseLogic);
        }
...
//
        ...
        And and=new And("A",tagCompare.getOutput(0),val.getOutput(0));
//        tagCompare==1:Data with given set has given tag
        tagCompare.setOutput(0,and.getOutput(0));
...
//            addOutput(tagCompare.getOutput(0));
        ...
        for (int i = 0; i <32 ; i++) {
            ...
            setOutput(i,checkHit[i].getOutput(0));
        }
//
//
//
...
//
        ...
        for (int i = 0; i < 16*4; i++) {
            ...
            for (int j = 0; j < 32; j++) {
                And and11=new And("ss",getInput(j),Simulator.trueLogic);
                memory.setInput(
                        j + 2, and11.getOutput(0));
            }
...
//                data[i/4].setInput(j+(i/16)*32,memory.getOutput(j));
//                        Mux2to1 choose = new Mux2to1("mux" + i + j, "3x1", getInput(32), getInput(33), memory.getOutput(k));
